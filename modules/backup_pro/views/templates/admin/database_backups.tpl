<div id="container" class="row">
    ...
    <div class="panel">
		...
		<tbody>
		<tr>
			<td width='120'> {$backup_meta['database']['total_backups']|escape:'htmlall':'UTF-8'} </td>
			<td width='150'>{$backup_meta['database']['total_space_used']|escape:'htmlall':'UTF-8'}</td>
	...
	<div class="row panel">
		...
		{if $backups['database']|count > 0}
			...
			<div class="panel-footer"><button name="_remove_backup_button" class="btn btn-primary pull-right" value="1" id="_remove_backup_button" type="submit">
								{'delete_selected'|m62Lang}
							</button>
			</div>
			</form>			
		{else}
			{'no_database_backups'|m62Lang} <a href="{$link->getAdminLink('AdminBackupProBackupDatabase')|escape:'html':'UTF-8'}">{'would_you_like_to_backup_database_now'|m62Lang}</a>
		{/if}
