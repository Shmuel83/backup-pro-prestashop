...
class Backup_ProCronModuleFrontController extends BaseController
{
    ...
    {
        if ($this->getPost('backup_pro') != $this->settings['cron_query_key']) {
            exit();
        }
    ...
    public function integrity()
    {
        ...
        $backup = $this->services['backups']->setLocations($this->settings['storage_details'])->setBackupPath($this->settings['working_directory']);
        $storage = $this->services['backup']->setStoragePath($this->settings['working_directory']);
        $backup->getIntegrity()
            ->setFile($this->services['files'])
            ->setStorage($storage->getStorage());
        ...
        foreach ($backups[$type] as $details) {
            if (empty($details['verified']) || $details['verified'] == '0') {
                if ($type == 'files') {
                    ...
                    $backup->getIntegrity()
                        ->setDbConf($this->platform->getDbCredentials())
                ...
                $this->services['backup']->getDetails()->addDetails($details['file_name'], $path, array(
                    'verified' => $status
                ));
            }
